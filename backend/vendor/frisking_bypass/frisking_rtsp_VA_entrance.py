...
def upload_event_to_server(upload_config, event_frame, bbox_xyxy, person_id, tracking_images=None):
    ...
    if not image_bytes:
        print(f"  UPLOAD ERROR: Failed to compress event image")
        return False
    
    # Generate thumbnail
    thumb_bytes = generate_thumbnail(event_frame, size=thumb_size, max_kb=max_thumb_kb)
    if not thumb_bytes:
        print(f"  UPLOAD ERROR: Failed to generate thumbnail")
        return False
...
class FriskingDetector:
    def __init__(self, model_path="yolo11l-pose.pt", person_model_path=None, confidence=0.5, max_frames=30, distance_threshold=100):
        ...
        self.frisking_time_threshold = float(getenv('FRISKING_TIME_SEC', '2.0')) # Configurable frisking time in seconds
        ...
        self.line_crossing_grace_frames = max(1, int(getenv('LINE_CROSSING_GRACE_FRAMES', '40')))
        ...
        self.debug_label = 'entrance'
        self.debug_log(
            (
                "crossing direction=TOP->BOTTOM "
    ...
    def draw_horizontal_line(self, frame):
        ...
        if self.horizontal_line_y is None or self.horizontal_line_frame_shape != frame_shape:
            self.horizontal_line_y = int(frame.shape[0] * 0.80) # 70% - catches feet crossing the line
        ...
        cv2.putText(frame, "FRISKING LINE (EXIT ->)", (10, label_y), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        # Draw downward arrow to show detection direction
    ...
    def downward_motion_metrics(self, samples):
        if len(samples) < 2:
            return 0.0, 0.0

        total_delta = float(samples[-1]) - float(samples[0])
    ...
    def line_crossing_direction_ok(self, foot_samples, center_samples, top_samples):
        foot_delta, foot_consistency = self.downward_motion_metrics(foot_samples)
        center_delta, center_consistency = self.downward_motion_metrics(center_samples)
        top_delta, top_consistency = self.downward_motion_metrics(top_samples)
        direction_ok = (
    ...
    def check_line_crossing(self, person_id, current_y, bbox=None):
        """Confirm an entrance crossing only after a full top-to-bottom traversal."""
    ...
    def emit_missed_frisking_event(self, frame, person_id, track, missed_frisking_events, event_frame=None, event_bbox=None):
        bbox = event_bbox or self.latest_track_bbox(track or {})
        if not bbox or bbox[2] <= 10 or bbox[3] <= 10:
            self.debug_log(
        ...
        filename = self.save_missed_frisking_frame(clean_frame, bbox, person_id)
        ...
        self.debug_log(
            f"MISSED event emitted bbox={bbox} saved={filename}",
            person_id,
            force=True
        )
    ...
    def resolve_lost_pending_crossings(self, frame, missed_frisking_events, previous_track_events, frame_count):
        ...
        for person_id, crossed_at in list(self.pending_line_crossings.items()):
            ...
            lost_age = frame_count - track.get('last_seen', crossed_at) if track else frames_since_crossing
            ...
            counts = self.evidence_counts(person_id, self.recent_interaction_window)
            was_frisked = person_id in self.frisked_people or self.has_recent_frisking_interaction(person_id)
            ...
            if was_frisked:
                ...
                self.debug_log(
                    (
                        "FRISKED decision after track loss "
                        f"recent_pose={counts['pose_frisking']}/{self.strong_pose_latch_frames} "
                        f"recent_near_still={counts['near_still']}/{self.min_near_still_frames} "
                        f"recent_score={counts['frisking_score']}/{self.frisking_score_threshold} "
                        f"max_reach={counts['max_reach_score']}/{self.frisking_strong_reach_score_threshold}"
    ...
    def process_single_frame(self, frame):
            ...
            if self.track_visible_this_frame(track, frame_count):
                ...
                if p_bbox:
                    ...
                    if (
                        ...
                        near_still = sum(1 for h in hist if h['near_security'] and h['standing_still'])
                        pose_cnt = sum(1 for h in hist if h['pose_frisking'])
                        score_cnt = sum(int(h.get('frisking_score', 0) or 0) for h in hist)
                        label = f"{pid} WAIT:{remaining} S:{near_still} P:{pose_cnt} F:{score_cnt}"
                        frame = self.draw_bbox_with_label(frame, p_bbox, label, color=(0, 165, 255), thickness=2) # Orange
                    else:
        ...
        for person_id, track in self.person_tracks.items():
            ...
            # evidence so a single false security hit does not suppress a missed event.
            if (
            ...
            self.debug_log(
                (
                    ...
                    f"score={counts['frisking_score']}/{self.frisking_score_threshold} "
                    ...
                    if self.mark_frisked_if_confirmed(person_id, "at crossing"):
                        ...
                        self.debug_log(
                            (
                                ...
                                f"line_y={self.horizontal_line_y}; already FRISKED"
                            ),
                            person_id,
                            force=True
                        )
                    elif person_id not in self.pending_line_crossings:
            ...
            # 4. Check if grace period has expired for any pending crossing
            if person_id in self.pending_line_crossings:
                ...
                if frames_since_crossing >= grace_frames:
                    ...
                    # Grace period expired! NOW make the final decision using ALL evidence
                    del self.pending_line_crossings[person_id]
                    self.pending_crossing_grace_frames.pop(person_id, None)
                    ...
                    if was_frisked:
                        ...
                        self.debug_log(
                            (
                                "FRISKED decision "
                                f"recent_pose={counts['pose_frisking']}/{self.strong_pose_latch_frames} "
                                f"near_still={counts['near_still']}/{self.min_near_still_frames} "
                                f"recent_score={counts['frisking_score']}/{self.frisking_score_threshold} "
                                f"max_reach={counts['max_reach_score']}/{self.frisking_strong_reach_score_threshold}"
                            ),
                            person_id,
...
def open_rtsp_stream(rtsp_url, retries=5, delay=3):
    ...
    for attempt in range(1, retries + 1):
        ...
        # Tune RTSP transport and buffer for lower latency
...
def main():
    # Load API upload config
    upload_config = load_upload_config()

    # ── Hardcoded configuration ──────────────────────────────────────
    rtsp_url = "rtsp://admin:Admin@123@192.168.121.3:554/video/live?channel=1&subtype=0"
    model_path = "yolo11l-pose.pt"
    reconnect_delay = 5  # seconds to wait before reconnecting on stream loss
    # ─────────────────────────────────────────────────────────────────
    ...
    cv2.namedWindow('Frisking Missed - RTSP', cv2.WINDOW_NORMAL)
    cv2.resizeWindow('Frisking Missed - RTSP', 1280, 720)

    consecutive_failures = 0
    max_consecutive_failures = 30  # Reconnect after this many consecutive read failures
    ...
    while True:
        ...
        if total_elapsed > 0:
            avg_fps = total_frame_count / total_elapsed
        ...
        cv2.putText(processed_frame, f"Avg FPS: {avg_fps:.1f}", (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

        # processed_frame already has the frisking line and bounding boxes drawn by the detector
        cv2.imshow('Frisking Missed - RTSP', processed_frame)
